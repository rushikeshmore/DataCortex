...
fn main() {
    ...
    match value_dict::preprocess(&col_data) {
        Some(result) => {
            ...
            eprintln!("  Total: {} bytes", result.data.len() + result.metadata.len());
            eprintln!("  Savings: {} bytes ({}%)", 
            ...
            if recovered == col_data {
                ...
                eprintln!("  Roundtrip: FAILED (recovered {} bytes vs {} original)", recovered.len(), col_data.len());
            }
        }
        None => {
